  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains(STORE_NAMES.MEASUREMENTS)) {
        ...
        store.createIndex("batteryId", "batteryId", { unique: false });
        store.createIndex("date", "date", { unique: false });
        store.createIndex("type", "type", { unique: false });
        store.createIndex("source", "source", { unique: false });
        store.createIndex("createdAt", "createdAt", { unique: false });
        store.createIndex("batteryId_date", ["batteryId", "date"], { unique: false });
      }
...
async function ensureDefaultRecords() {
  ...
  const databaseMetadata = await getByKey(STORE_NAMES.METADATA, "database");
...
export async function getBatteryById(id) {
  return getByKey(STORE_NAMES.BATTERIES, id);
}

export async function saveBattery(battery) {
  return put(STORE_NAMES.BATTERIES, battery);
}

export async function deleteBattery(id) {
  return remove(STORE_NAMES.BATTERIES, id);
}

export async function getMeasurementsByBatteryId(batteryId) {
  const store = transaction(STORE_NAMES.MEASUREMENTS);
  const index = store.index("batteryId");
  const measurements = await requestToPromise(index.getAll(batteryId));

  return measurements.sort((a, b) => a.date.localeCompare(b.date));
}

export async function saveMeasurement(measurement) {
  return put(STORE_NAMES.MEASUREMENTS, measurement);
}

export async function deleteMeasurement(id) {
  return remove(STORE_NAMES.MEASUREMENTS, id);
}
...
export async function saveSettings(settings) {
  return put(STORE_NAMES.SETTINGS, settings);
}
...
export async function replaceAllData(data) {
  await clearStore(STORE_NAMES.BATTERIES);
  await clearStore(STORE_NAMES.MEASUREMENTS);
  await clearStore(STORE_NAMES.SETTINGS);
  await clearStore(STORE_NAMES.METADATA);
