...
function renderMeasurementHistory(measurementsWithRates) { const visible = measurementsWithRates.slice(-10).reverse(); return `${visible.map(m => `<div class="history-item" data-measurement-id="${m.id}"><span>${m.date}</span><strong>${m.levelPercent} %</strong><span class="history-rate">${m.rateLabel}</span></div>`).join("")}${measurementsWithRates.length > 5 ? `<p class="helper-text">10 dernières mesures affichées.</p>` : ""}`; }
